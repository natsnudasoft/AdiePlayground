    ...
    public class DbContextCollection : IDbContextCollection
    {
        ...
        public TDbContext Get<TDbContext>() where TDbContext : DbContext
        {
            ...
            if (!_initializedDbContexts.ContainsKey(requestedType))
            {
                ...
                RegisterSaveChangesCheck(dbContext);
        ...
        public int Commit()
        {
            ...
            foreach (var dbContext in _initializedDbContexts.Values)
            {
                try
                {
                    if (!_readOnly)
                    {
                        UnregisterSaveChangesCheck(dbContext);
        ...
        private void RegisterSaveChangesCheck(DbContext dbContext)
        {
            ObjectContext objectContext = (dbContext as IObjectContextAdapter)?.ObjectContext;
            if (objectContext != null)
            {
                objectContext.SavingChanges += ObjectContextSavingChanges;
            }
        }

        private void UnregisterSaveChangesCheck(DbContext dbContext)
        {
            ObjectContext objectContext = (dbContext as IObjectContextAdapter)?.ObjectContext;
            if (objectContext != null)
            {
        ...
        private void ObjectContextSavingChanges(object sender, EventArgs e)
        {
            throw new InvalidOperationException("Do not call context.SaveChanges on a context managed by a DbContextScope.");
        }
